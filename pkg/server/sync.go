...
var stopEnumeratingError = errors.New("sentinel error")

// hourlyCompare fetches blobs from a random point in the destination every hour,
// downloads and verifies them, until it downloaded hourlyBytes.
func (sh *SyncHandler) hourlyCompare(hourlyBytes uint64) {
	...
	for {
		...
		err := blobserver.EnumerateAllFrom(context.TODO(), sh.from, after, func(sr blob.SizedRef) error {
			...
			if size != sr.Size {
				return fmt.Errorf("%s: expected size %d, got %d", sr.Size, size)
			}
			if err != nil {
		...
		if err != nil && err != stopEnumeratingError {
			sh.compareErrors = append(sh.compareErrors, err.Error())
